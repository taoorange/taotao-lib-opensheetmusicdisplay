...
export abstract class MusicSystem extends GraphicalObject {
    ...
    /** Labels by instrument (one label per instrument in this system). */
    protected labels: Map<Instrument, GraphicalLabel> = new Map<Instrument, GraphicalLabel>();
    ...
    public createMusicSystemLabel(  instrumentLabelTextHeight: number, systemLabelsRightMargin: number,
        ...
        for (let idx: number = 0, len: number = this.staffLines.length; idx < len; ++idx) {
            ...
            if (isFirstSystem) {
                ...
                // For all following systems, prefer abbreviations but gracefully fall back
                // to full instrument names so every system still shows instrument labels.
                const useAbbreviation: boolean =
                    this.rules.RenderPartAbbreviations &&
                    this.staffLines.length > 1 &&
                    !!instrument.PartAbbreviation;
                const labelText: string = useAbbreviation ? instrument.PartAbbreviation : instrument.NameLabel.text;
            ...
            if (instrument?.NameLabel?.print) {
                ...
                this.labels.set(instrument, graphicalLabel);
        ...
        this.maxLabelLength = 0.0;
        const labels: GraphicalLabel[] = Array.from(this.labels.values());
    ...
    public setMusicSystemLabelsYPosition(): void {
        this.labels.forEach((value: GraphicalLabel, key: Instrument): void => {
