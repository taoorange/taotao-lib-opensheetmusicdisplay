...
(function () {
    ...
    function init() {
        ...
        var paramPaged = findGetParameter('paged');
        var paramPagedFormat = findGetParameter('pagedFormat');
        ...
        const pagedEnabled = paramPaged === '1' || paramPaged === 'true';
        // Keep old behavior by default (Endless). If paged=1 is provided, force a page format
        // unless caller already set one via pageFormat/pageWidth/pageHeight.
        let pageFormat = pagedEnabled ? (paramPagedFormat || "A4_P") : "Endless";
        if (paramPageFormat) {
            pageFormat = paramPageFormat;
        }
        ...
        if (openFileBtn && openFileInput) {
            ...
            openFileInput.onchange = function (e) {
    ...
    function selectSampleOnChange(input) {
        error();
        disable();
        var isCustom = input !== undefined && input !== null;
        var loadInput = input;
        if (!isCustom) {
            if (selectSample) {
                loadInput = sampleFolder + selectSample.value;
            } else {
                if (samples && samples.length > 0) {
                    loadInput = sampleFolder + samples[0];
                } else {
        ...
        const samplePath = typeof loadInput === "string" ? loadInput : "";
        setSampleSpecificOptions(samplePath, isCustom);

        openSheetMusicDisplay.load(loadInput).then(
            function () {
    ...
    function loadLocalFile(file) {
        ...
        if (lowerFilename.endsWith(".xml")
            || lowerFilename.endsWith(".musicxml")
            || lowerFilename.endsWith(".mxl")) {
            selectSample.appendChild(custom);
            custom.selected = "selected";
            // Let OSMD decide whether this is zipped MXL or plain XML.
            selectSampleOnChange(file);
            return;
        }
